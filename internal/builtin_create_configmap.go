...
func (f *CreateConfigMap) Configure(logger *log.Logger, pwd, _, _ string) error {
	f.logger = logger
	f.pwd = pwd
	return nil
}

func (f *CreateConfigMap) Invoke(r io.Reader, w io.Writer) error {
	if f.Name == "" {
	...
	for i, content := range f.Contents {
		...
		if content.Value != "" {
			...
			if !filepath.IsAbs(content.Path) {
				path = filepath.Join(f.pwd, content.Path)
			}
	...
	pipeline := kio.Pipeline{
		...
		Filters: []kio.Filter{pkg.Generate(func() ([]*yaml.RNode, error) {
			...
			node, err := yaml.NewMapRNode(nil).Pipe(
				...
				yaml.Tee(yaml.SetAnnotation(pkg.PreviousNameAnnotationName, f.Name)),
