...
func (f *dockerFunction) pullImage(ctx context.Context, dockerClient *client.Client) error {
	...
	} else if len(images) == 0 || (len(images) == 1 && isLatest(&images[0])) {
...
func (f *dockerFunction) collectBindsFromConfig(v reflect.Value) error {
	...
	case reflect.Array, reflect.Slice:
		for i := 0; i < v.Len(); i++ {
			err := f.collectBindsFromConfig(v.Index(i))
			if err != nil {
				return fmt.Errorf("failed collecting mount files: %w", err)
			}
		}
		return nil
	case reflect.Map:
		for _, k := range v.MapKeys() {
			err := f.collectBindsFromConfig(v.MapIndex(k))
			if err != nil {
				return fmt.Errorf("failed collecting mount files: %w", err)
			}
		}
		return nil
	case reflect.String:
		subMatches := f.bindsRegexp.FindStringSubmatch(v.String())
		if subMatches != nil {
			if subMatches[2] == "" {
				f.binds = append(f.binds, subMatches[1]+":/"+subMatches[1])
			} else {
				f.binds = append(f.binds, subMatches[1]+":"+subMatches[2])
			}
		}
		return nil
	default:
...
func (f *dockerFunction) createContainer(ctx context.Context, dockerClient *client.Client, configFile string) (string, func(), error) {
	//err := f.collectBindsFromConfig(reflect.ValueOf(f.config))
